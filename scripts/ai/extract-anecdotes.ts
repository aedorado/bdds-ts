...
import { extractAnecdotes } from '@/lib/ai'
import { eq } from 'drizzle-orm'

/**
 * Script to extract anecdotes from a lecture
 * Usage: npx tsx scripts/ai/extract-anecdotes.ts --lectureId=123
 */
async function main() {
  ...
  if (!lectureIdArg) {
    console.error('Usage: npx tsx scripts/ai/extract-anecdotes.ts --lectureId=123')
    process.exit(1)
  }
  ...
  try {
    console.log(`📚 Extracting anecdotes from lecture ${lectureId}...`)
    ...
    if (existing.length) {
      await db
        .update(aiSummaries)
        .set({ anecdotes, generatedAt: new Date() })
        .where(eq(aiSummaries.lectureId, lectureId))
    } else {
    ...
    console.log('✅ Anecdotes extracted successfully!')
    console.log(`\nExtracted ${anecdotes.length} anecdotes:`)
    console.log('---')
    anecdotes.forEach((anecdote, idx) => {
      console.log(`${idx + 1}. ${anecdote}`)
    })
    console.log('---')
  } catch (error) {
    console.error('Error extracting anecdotes:', error)
    process.exit(1)
  }
}
