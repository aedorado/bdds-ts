...
import { extractThemes } from '@/lib/ai'
import { eq } from 'drizzle-orm'

/**
 * Script to extract themes from a lecture
 * Usage: npx tsx scripts/ai/extract-themes.ts --lectureId=123
 */
async function main() {
  ...
  if (!lectureIdArg) {
    console.error('Usage: npx tsx scripts/ai/extract-themes.ts --lectureId=123')
    process.exit(1)
  }
  ...
  try {
    console.log(`📚 Extracting themes from lecture ${lectureId}...`)
